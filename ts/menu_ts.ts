...
function openPopUp(dataType, element) {
  ...
  switch (dataType) {
    ...
    case "mapping":
      ...
      // buffer
      var dbKey:string = "buffer"
      var text:string[] = ["{{.settings.streamBuffering.info_global}}", "{{.settings.streamBuffering.info_false}}", "xTeVe: ({{.settings.streamBuffering.info_xteve}})", "FFmpeg: ({{.settings.streamBuffering.info_ffmpeg}})", "VLC: ({{.settings.streamBuffering.info_vlc}})"]
      var values:string[] = ["", "-", "xteve", "ffmpeg", "vlc"]
      var select = content.createSelect(text, values, data[dbKey], dbKey)
      select.setAttribute("onchange", "javascript: this.className = 'changed'")
